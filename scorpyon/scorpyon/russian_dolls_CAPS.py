...
import pandas as pd
import nippy.ulist as ulist
import typyst.trans as pstr_trans
import scorpyon.manip as aw_manip
...
def split_dolls(df):
    return df.groupby(['ad_group'],group_keys=False).apply(lambda x:_split_fun(x))

def _split_fun(grp):
    d = pd.concat([_rep_ad_group_name(grp[grp.match_type==match],match) for match in _match_type_list])
    d['target'] = d.match_type
    return d
